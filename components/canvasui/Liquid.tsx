...
export interface LiquidOptions {
  ...
  rainbow?: boolean;
  /** Never capture HTML into the sim — draw the fluid as a pure overlay. */
...
export interface LiquidProps extends LiquidOptions {
  ...
  /**
   * Skip the experimental html-in-canvas capture and render children as
   * normal DOM with the fluid drawn on an overlay canvas. Required when
   * children rely on IntersectionObserver/scroll-reveal animations, which
   * never fire inside a canvas subtree.
   */
  overlay?: boolean;
}
...
export function Liquid({
  ...
  const native = !overlay && supported && !failed;
  ...
  useEffect(() => {
    ...
    instanceRef.current = createLiquid(
      ...
      { ...initialOptions, disableCapture: overlay || initialOptions.disableCapture },
    );
